import gradio as gr
from src.agents import ResearcherAgent # type: ignore
import logging
import sys
...
system = ResearcherAgent()

async def initialize_the_ResearcherAgent():
...
with gr.Blocks() as ui:
    gr.Markdown("## Researcher Chat Interface")
    status = gr.Markdown('Initializing')
    chat = gr.Chatbot()
    msg = gr.Textbox()
    msg.submit(run, [msg, chat], [chat, msg])
    ui.load(fn=initialize_the_ResearcherAgent, inputs=None, outputs=status)
